...
{
    // Convert RNRC:// to RNRC:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if (uri.startsWith("RNRC://")) {
        uri.replace(0, 12, "RNRC:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "RNRC.lnk";
}
...
{
    return GetAutostartDir() / "RNRC.desktop";
}
...
{
    header = tr("RNRC-Qt") + " " + tr("version") + " " +
             QString::fromStdString(FormatFullVersion()) + "\n\n" +
             tr("Usage:") + "\n" +
             "  RNRC-qt [" + tr("command-line options") + "]                     " + "\n";
